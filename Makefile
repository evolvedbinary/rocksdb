...
rocksdbjavastaticpublishcentral: rocksdbjavageneratepom
	cd java && mvn org.apache.maven.plugins:maven-gpg-plugin:3.0.1:sign-and-deploy-file -Durl=https://central.sonatype.com/api/v1/publisher -DrepositoryId=central -DpomFile=pom.xml -Dfile=target/rocksdbjni-$(ROCKSDB_JAVA_VERSION).jar
	$(foreach classifier, $(ROCKSDB_JAVA_RELEASE_CLASSIFIERS), cd java && mvn org.apache.maven.plugins:maven-gpg-plugin:3.0.1:sign-and-deploy-file -Durl=https://central.sonatype.com/api/v1/publisher -DrepositoryId=central -DpomFile=pom.xml -Dfile=target/rocksdbjni-$(ROCKSDB_JAVA_VERSION)-$(classifier).jar -Dclassifier=$(classifier);)

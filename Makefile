...
rocksdbjavastaticdockers390x: rocksdbjavanativedir
	docker run --rm --name rocksdb_linux_s390x-be --platform linux/s390x --attach stdin --attach stdout --attach stderr --volume $(HOME)/.m2:/root/.m2 --volume `pwd`:/rocksdb-host:ro --volume /rocksdb-local-build --volume `pwd`/$(JAVA_NATIVE_DIR):/rocksdb-java-target --env DEBUG_LEVEL=$(DEBUG_LEVEL) --env J=$(J) evolvedbinary/rocksjava:centos7_s390x-be /rocksdb-host/java/crossbuild/docker-build-linux.sh
...
rocksdbjavastaticdockerriscv64musl:
	docker run --rm --name rocksdb_linux_riscv64-musl-be --platform linux/riscv64 --attach stdin --attach stdout --attach stderr --volume $(HOME)/.m2:/root/.m2 --volume `pwd`:/rocksdb-host:ro --volume /rocksdb-local-build --volume `pwd`/$(JAVA_NATIVE_DIR):/rocksdb-java-target --env DEBUG_LEVEL=$(DEBUG_LEVEL) --env J=$(J) evolvedbinary/rocksjava:alpine3_riscv64-be /rocksdb-host/java/crossbuild/docker-build-linux.sh

  ...
  if constexpr (sizeof(CoeffRow) < 8) {
    ...
    for (ConstructionFailureChance cs : TypeParam::FailureChanceToTest()) {
      ...
      for (uint32_t i = 0; i < FLAGS_thoroughness; ++i) {
        ...
        // Compute num_slots, and re-adjust num_to_add to get as close as
        // possible to next num_slots, to stress that num_slots in terms of
        // construction success. Ensure at least one iteration:
        ...
        for (;;) {
          ...
          EXPECT_EQ(next_num_slots,
                    SimpleSoln::RoundUpNumSlots(next_num_slots));
        ...
        if (FLAGS_verbose) {
          fprintf(stderr,
                  "Adding(%s) %u / %u   Overhead: %g   Batch size: %u\n",
        ...
        bool batch_success;
        {
          ...
          if (TypeParam::kHomogeneous || overhead_ratio < 1.01 ||
            ...
            // extra check that after backtracking there are no remnants (e.g.
            // in result side of banding) of these entries.
          ...
          if (reseeds > log2_thoroughness + 1) {
            fprintf(stderr, "%s high reseeds at %u, %u/%u: %u\n",
                    reseeds > log2_thoroughness + 8 ? "ERROR Extremely"
                                                    : "Somewhat",
        ...
        if (first_single) {
          ...
          ASSERT_TRUE(!test_interleaved ||
                      isoln.FilterQuery(*one_more, hasher));
        }
        if (second_single) {
          ASSERT_TRUE(soln.FilterQuery(*two_more, hasher));
          ASSERT_TRUE(!test_interleaved ||
                      isoln.FilterQuery(*two_more, hasher));
        }
        if (batch_success) {
          cur = batch_begin;
          while (cur != batch_end) {
      ...
      // "outside" == key not in original set so either negative or false
      // positive
      fprintf(stderr,
      ...
      if (TypeParam::kHomogeneous) {
        ...
        // success rate (average of 1.0 re-seeds) or slightly better. But 1.15
        // is also close enough.
      ...
      if (total_singles > 0) {
        double single_failure_rate =
            1.0 * total_single_failures / total_singles;
